...
常量池就类似一个Java系统级别提供的缓存，8种基本数据类型的常量池都是系统协调的，所以我们对其进行太多考虑。String类型的常量池比较特殊，它的只要使用方法有两种：
...
7. String的intern()方法的小结：
   ...
   **注意：**什么时候会出现字符串常量池中没有该字符串，但是堆中有这个字符串呢？StringBuilder的toString()方法，它虽然返回了return new String(value, 0, count);但是没有在字符串常量池中常见字符串对象。所以需要在调用以下intern()方法，将这个字符串加到常量池中。
